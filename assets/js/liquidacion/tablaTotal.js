const mostrarTablaTotal = async () => {
  const datos = await obtenerTablaTotalValores();
  const serviceData = datos[0];
  ...
  const auNetoGrs = (auValorMetalGrms - auValorMetalGrms * (auDescuento / 100)).toFixed(2);
  ...
  const agNetoGrs = (agValorMetalGrms - agValorMetalGrms * (agDescuento / 100)).toFixed(2);
  ...
  const auRefinacionTotal = (oroFino * parseFloat(serviceData[1].price)).toFixed(2);
  const agRefinacionTotal = (plataFina * parseFloat(serviceData[2].price)).toFixed(2);
  ...
  const tablaTotalBody = `
    <tr id="analisisFinalAU">
      <td class="text-center">
        <span class="btn btn-icon text-primary" onclick="editarAnalisisFinal('AU')">
    ...
    <tr id="analisisFinalAG">
      <td class="text-center">
        <span class="btn btn-icon text-primary" onclick="editarAnalisisFinal('AG')">
          <i class="mdi mdi-pencil"></i>
        </span>
      </td>
    ...
    <tr>
      ...
      <td class="text-center">${parseFloat(serviceData[0].price).toFixed(3)}</td>
    ...
    <tr>
      ...
      <td class="text-center">${oroFino}</td>
      <td class="text-center">${parseFloat(serviceData[1].price).toFixed(3)}</td>
    ...
    <tr>
      ...
      <td class="text-center">${plataFina}</td>
      <td class="text-center">${parseFloat(serviceData[2].price).toFixed(3)}</td>
    ...
    <tr>
      ...
      <td class="text-center">${parseFloat(serviceData[3].price).toFixed(3)}</td>
    ...
    <tr>
      ...
      <td class="text-center"></td>
      <td class="text-center">${parseFloat(serviceData[4].price).toFixed(3)}</td>
...
const editarAnalisisFinal = (id) => {
  ...
  if (fila) {
    ...
    fila.cells[0].innerHTML = `
      ...
      `;
    fila.cells[2].innerHTML = '<input disabled type="text" id="seco-' + id + '" value="' + pesoGrs + '" onkeyup="leyes(' + id + ')">';
    fila.cells[3].innerHTML = '<input type="text" id="officeLaw-' + id + '" value="' + accion_inter_onza + '" onkeyup="leyes(' + id + ')">';
    fila.cells[4].innerHTML = '<input disabled type="text" id="clientLaw-' + id + '" value="' + accion_inter_grs + '" onkeyup="leyes(' + id + ')">';
    fila.cells[5].innerHTML = '<input disabled type="text" id="difference-' + id + '" value="' + valorGrs + '" onkeyup="leyes(' + id + ')">';
    fila.cells[6].innerHTML = '<input type="text" id="final-' + id + '" value="' + descuento + '">';
    fila.cells[7].innerHTML = '<input disabled type="text" id="neto-' + id + '" value="' + netoGrs + '">';
  }
};
...
const guardarAnalisisFinal = async (id) => {
  ...
  if (fila) {
    const analisisData = {
      element: id,
      accion_inter_onza: parseFloat(fila.cells[3].getElementsByTagName('input')[0].value),
      descuento: parseFloat(fila.cells[6].getElementsByTagName('input')[0].value),
    };
    ...
    $.ajax({
      ...
      .done(function (datos) {
        console.log(datos);
        fila.cells[0].innerHTML = `
            <span class="btn btn-icon text-primary" onclick="editarAnalisisFinal('${id}')">
              <i class="mdi mdi-pencil"></i>
            </span>
          `;
