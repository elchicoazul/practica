const renderElementsAnalisis = (data, tableId) => {
  ...
  const entregar = tableId === 'tablaAnalisisAG' ? 'porEntregarAG' : 'porEntregarAU';
  ...
  leyGeneralCola = leyGeneralCola === '0.0000' ? (tableId === 'tablaAnalisisAG' ? tailings_law : gold_law) : parseFloat(leyGeneralCola);
  const leyGeneralColaTotal = (pesoSecoTotal * (leyGeneralCola / 100)).toFixed(2);
  ...
  elementoPorEntregar =
    elementoPorEntregar === '0.0000' ? (tableId === 'tablaAnalisisAG' ? pine_silver_to_deliver : fine_gold_to_deliver) : parseFloat(elementoPorEntregar);
  const elementoPorEntregarTotal = (netoTotalMenosLey * (elementoPorEntregar / 100)).toFixed(2);
  ...
  const total = `
      ...
      <tr id="fila-${ley}">
        <th>
          <span class="btn btn-icon text-primary"  onclick="editarAnalisisTotal('${ley}')">
            <i class="mdi mdi-pencil"></i>
          </span>
        </th>
        <th></th>
      ...
      <tr id="fila-${entregar}">
        <th>
        <span class="btn btn-icon text-primary" onclick="editarAnalisisTotal('${entregar}')">
            <i class="mdi mdi-pencil"></i>
          </span>
        </th>
        <th></th>
...
function editarAnalisisTotal(filaId) {
  ...
  if (fila) {
    ...
    fila.cells[5].innerHTML = '<input type="text" id="final-' + filaId + '" value="' + finalValor + '">';
  }
}
...
const guardarAnalisisTotal = async (filaId) => {
  const userId = $('#cliente').val();
  var fila = document.getElementById('fila-' + filaId);
  const tipoLey = filaId === 'leyGenAG' || filaId === 'leyGenAU' ? parseFloat(fila.cells[5].getElementsByTagName('input')[0].value) : null;
  const tipoEntrega = filaId === 'porEntregarAG' || filaId === 'porEntregarAU' ? parseFloat(fila.cells[5].getElementsByTagName('input')[0].value) : null;

  const porcentajeValor = [];
  if (filaId === 'porEntregarAU') {
    porcentajeValor[0] = document.getElementById('fila-leyGenAU').cells[5].innerText;
    porcentajeValor[0] = parseFloat(porcentajeValor[0]);
  }
  if (filaId === 'leyGenAU') {
    porcentajeValor[1] = document.getElementById('fila-porEntregarAU').cells[5].innerText;
    porcentajeValor[1] = parseFloat(porcentajeValor[1]);
  }
  if (filaId === 'porEntregarAG') {
    porcentajeValor[2] = document.getElementById('fila-leyGenAG').cells[5].innerText;
    porcentajeValor[2] = parseFloat(porcentajeValor[2]);
  }
  if (filaId === 'leyGenAG') {
    porcentajeValor[3] = document.getElementById('fila-porEntregarAG').cells[5].innerText;
    porcentajeValor[3] = parseFloat(porcentajeValor[3]);
  }

  if (fila) {
    const analisisData = {
      element: filaId === 'leyGenAG' || filaId === 'porEntregarAG' ? 'AG' : 'AU',
      element_law: tipoLey === null ? porcentajeValor.filter((e) => e > 0)[0] : tipoLey,
      element_to_deliver: tipoEntrega === null ? porcentajeValor.filter((e) => e > 0)[0] : tipoEntrega,
    };
    ...
    $.ajax({
      ...
      .done(function (datos) {
        // console.log(datos);
        fila.cells[0].innerHTML = `
            <span class="btn btn-icon text-primary" onclick="editarAnalisisTotal(${filaId})">
              <i class="mdi mdi-pencil"></i>
            </span>
          `;
        fila.cells[5].innerHTML = fila.cells[5].getElementsByTagName('input')[0].value;
